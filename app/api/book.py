...
from app.schemas.book import BookSetupSubmitSchema, BookCreate,BookSetupRequest, BookUpdate, BookListSchema
...
from app.crud.book import create_book, update_book
...
from app.services.common import safe_json_loads
...
def run_conduct_research_worker(book_id: UUID):
    ...
    try:
        ...
        research_sources = (
            db.query(ResearchSource.source_url, ResearchSource.source_site)
        ...
        research_sources = {
            site: url
            for url, site in research_sources
            if url
        }

        

        research_sources_lst = [
            url
            for site, url in research_sources.items()
            if site not in ["linkedin", "twitter","youtube"]
        ]

        linkedin=research_sources.get("linkedin")
        twitter=research_sources.get("twitter")
        youtube=research_sources.get("youtube")


        logger.info(f"🔍 Research sources: {research_sources}")
        logger.info(f"🔍 Research sources lst: {research_sources_lst}")
        logger.info(f"🔍 linkedin and twitter: {linkedin} , {twitter}")
        ...
        if isinstance(outline, str):
            outline = safe_json_loads(outline)
...
def update_book_from_setup(
    ...
    try:
        ...
        # 1️⃣ Fetch BookUser
        logger.info(f"payload at update book setup:{payload}")
        book_setup = payload.book_setup
        book_user = (
            db.query(BookUser)
            .filter(
        ...
        logger.info(f"book_user at update book setup:{book_user.id}")
        ...
        if not last_book:
            ...
            logger.info("Book not found to update")
            last_book = create_book(
            db=db,
            data=BookCreate(
                book_user_id=book_user.id,
                genre=genre,
            ...
            new_created=True
            logger.info(f"book created with id:{last_book.id} at update api")
        ...
        if not new_created:
            ...
            logger.info(f"book updated with id:{book.id}")
...
@router.get("/last-book-data", status_code=status.HTTP_201_CREATED,response_model=BookSetupRequest)
def get_last_book(
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db),
):
    ...
    if not last_book:
        logger.info("No book found for this user")
    ...
    return {
        "book_setup": {
            ...
            "custom_genre": None,
            "working_title": last_book.title,
            "chapter_count": last_book.number_of_chapters,
            "desired_length": last_book.length_type,
            "dedication": last_book.dedication,
            "gdpr_consent": True
        }
    }
@router.get(
...
def get_dummy_book_setup():
    ...
    return {
        "book_setup": {
            "genre": "professional",
            "custom_genre": None,
            "working_title": "Building Products with Purpose modified done edited kbdfsd",
            "chapter_count": 12,
...
@router.get(
    "/list",
    response_model=List[BookListSchema]
)
def get_all_books(current_user=Depends(get_current_user),
    db: Session = Depends(get_db),):
    """"
    API ENDPOINT TO GET ALL BOOKS FOR A USER
    """
    try:
        logger.info("get_all_books api called")
        book_users = (
            db.query(BookUser)
            .filter(
                BookUser.user_id == current_user.id,
        ...
        logger.info(f"book list fetched:{books}")

        book_list = []
        for book in books:
            book_list.append({
                "id": book.id,
                "working_title": book.title,
                "status": book.status,
                "genre": book.genre,
                "dedication": book.dedication,
                "chapter_count": book.number_of_chapters,
                "desired_length": book.length_type,
                "gdpr_consent": True
            })
        ...
        logger.info(f"failed to get all books:{str(e)}")
        return {
            []
        }
